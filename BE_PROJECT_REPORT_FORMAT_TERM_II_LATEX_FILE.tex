	...
	\subsubsection{Naive Baye's Classification Algorithm}
		 \begin{algorithm}
			...
			\begin{algorithmic}[1]
				...
				\State classifier = load\_naive\_classifier()
				\State emotion = classifier.classify(sentence)
...
\end{algorithm}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\State word\_list = lemmatize(word\_list)
			\State word\_dictionary = $\phi$
			\For{each word in word\_list}
			\State word\_emotion = NRCclassifier(word)
			\State word\_dictionary\{word\} = word\_emotion
			\EndFor
			\algstore{myalg}
		\end{algorithmic}
	\end{algorithm}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			\State emotion\_dictionary = \{'neutral':0,'joy':0,'sadness':0,'surprise':0,'anger':0\}
			...
			\State get\_index = find\_max\_qualified\_word(dependency\_tree)
			...
			\EndIf
			\EndFor
			\State Indices = Indices $\bigcup$ get\_index
			\EndFor
			\State answer\_emotion = get\_max\_valued\_emotion(emotion\_dictionary)
			\If {negation present in the sentence}
			\If {the negation word qualifies an index in Indices}
			\If {max\_emotion\_value $\>$ 20}
			\State answer\_emotion = 'neutral'
			\ElsIf {max\_emotion\_value == 20}
			\If {answer\_emotion == 'joy'}
			\State answer\_emotion = 'sadness'
			\ElsIf {answer\_emotion == 'sadness'}
			\State answer\_emotion = 'joy'
			\Else
			\State answer\_emotion = 'neutral'
			\EndIf
			\EndIf
